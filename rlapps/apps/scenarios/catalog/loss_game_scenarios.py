...
from rlapps.envs.loss_game_alpha_multi_agent_env import LossGameAlphaMultiAgentEnv
from rlapps.envs.loss_game_multi_agent_env import LossGameMultiAgentEnv
from rlapps.envs.multi_dim_loss_game_alpha_multi_agent_env import (
    LossGameMultiDimMultiAgentEnv,
)
...
for total_moves in [2, 5, 10]:
    for game_class in [
        LossGameMultiAgentEnv,
        LossGameAlphaMultiAgentEnv,
        LossGameMultiDimMultiAgentEnv,
    ]:
        ...
        game_variant = (
            "_alpha" if issubclass(game_class, LossGameAlphaMultiAgentEnv) else ""
        )
        game_variant = (
            "_multi_dim"
        ...
        base_psro = PSROScenario(
            ...
            env_config={"total_moves": total_moves},
        ...
        if issubclass(game_class, LossGameMultiDimMultiAgentEnv):
            ...
            for warm_start in [0, 5, 10]:
                for increase_multiplier in [1.2, 1.5, 2.0]:
                    scenario_catalog.add(
                        base_nxdo.with_updates(
                            name=f"loss_game_nxdo_{total_moves}_moves{game_variant}_max_move_1.5_warm_start_{warm_start}_coef_{increase_multiplier}",
                            env_config={
                                "total_moves": total_moves,
                                "max_move_amount": 1.5,
                            },
                            get_restricted_game_solver=lambda scenario, warm_start=warm_start, increase_multiplier=increase_multiplier: SolveRestrictedGameIncreasingTimeToSolve(
                                scenario=scenario,
                                dont_solve_first_n_nxdo_iters=warm_start,
