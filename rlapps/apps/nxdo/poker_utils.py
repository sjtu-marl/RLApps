...
def _parse_action_probs_from_action_info(
    action, action_info, legal_actions_list, total_num_discrete_actions
):
    action_probs = None
    ...
    if action_probs is None:
        if "behaviour_logits" in action_info:
            ...
            discrete_action = parse_discrete_poker_action_from_continuous_space(
...
def openspiel_policy_from_nonlstm_rllib_nxdo_policy(
    ...
    def policy_callable(state: pyspiel.State):
        ...
        if (
            openspiel_game.get_type().short_name
            in ["leduc_poker", "oshi_zumo", "oshi_zumo_tiny", "universal_poker"]
        ...
        if is_openspiel_restricted_game:
            os_restricted_game_convertor: AgentRestrictedGameOpenSpielObsConversions = (
                restricted_game_convertor
            )
            try:
        ...
        if is_openspiel_restricted_game:
            ...
            for restricted_game_action in range(len(restricted_game_action_probs)):
                (
                    sampled_base_game_action,
                    _,
                    action_info,
                ) = restricted_game_convertor.get_base_game_action(
                    obs=obs,
                ...
                base_game_action_probs_for_rstr_action = (
                    _parse_action_probs_from_action_info(
...
def nxdo_nfsp_measure_exploitability_nonlstm(
    ...
    for action_space_converter, rllib_policy in zip(
        restricted_game_convertors, rllib_policies
    ):
...
def nxdo_snfsp_measure_exploitability_nonlstm(
        ...
        for checkpoint_path_tuple in br_checkpoint_path_tuple_list:
            ...
            for player, (restricted_game_convertor, player_rllib_policy) in enumerate(
                ...
                single_openspiel_policy = (
                    openspiel_policy_from_nonlstm_rllib_nxdo_policy(
                        openspiel_game=openspiel_game,
    ...
    num_players = 2
    weights = np.ones(shape=(len(br_checkpoint_path_tuple_list), num_players)) / len(
        br_checkpoint_path_tuple_list
    )

    print(f"weights: {weights}")

    avg_policies = tabular_policies_from_weighted_policies(
        game=openspiel_game, policy_iterable=policy_iterable(), weights=weights
    )
