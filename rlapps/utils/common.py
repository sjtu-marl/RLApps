...
def data_dir() -> str:
    return os.path.join(os.path.dirname(rlapps.__file__), "data")


def assets_dir() -> str:
    return os.path.join(os.path.dirname(rlapps.__file__), "assets")
...
def string_to_int_tuple(s) -> Tuple[int]:
    return tuple(
        int(number)
        for number in s.replace("(", "").replace(")", "").replace("...", "").split(",")
    )
...
def copy_attributes(src_obj, dst_obj):
    if not isinstance(src_obj, type(dst_obj)) and not isinstance(
        dst_obj, type(src_obj)
    ):
