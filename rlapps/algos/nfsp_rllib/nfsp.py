...
from rlapps.algos.nfsp_rllib.nfsp_torch_avg_policy import NFSPTorchAveragePolicy
from rlapps.algos.nfsp_rllib.reservoir_replay_buffer import ReservoirReplayActor
...
def execution_plan(
    ...
    replay_op = (
        LocalIterator(gen_replay, SharedMetrics())
    ...
    replay_op = map(
        lambda x: x if not isinstance(x, _NextValueNotReady) else {}, replay_op
    )
...
def get_policy_class(config: TrainerConfigDict) -> Optional[Type[Policy]]:
    ...
    if config["framework"] == "torch":
        ...
        raise NotImplementedError(
            f"NFSP average policy for framework: {config['framework']} not implemented."
        )
...
def get_store_to_avg_policy_buffer_fn(
    nfsp_trainer: NFSPTrainer,
) -> Callable[[SampleBatchType], SampleBatchType]:
