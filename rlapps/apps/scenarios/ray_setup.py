...
def init_ray_for_scenario(
    scenario: RayScenario, head_address: str = None, logging_level=logging.INFO
) -> Union[str, None]:
    if (
        head_address is not None
        or scenario.ray_object_store_memory_cap_gigabytes is None
    ):
        object_store_memory_bytes = None
    else:
        object_store_memory_bytes = int(
            1073741824 * scenario.ray_object_store_memory_cap_gigabytes
        )
