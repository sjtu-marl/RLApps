...
from rlapps.utils.common import check_if_jsonable
...
class StrategySpec(object):
    def __init__(self, strategy_id: str, metadata: dict, pure_strategy_indexes=None):
        ...
        if not is_jsonable:
            raise ValueError(
                f"Metadata is not JSON serializable. "
        ...
        if pure_strategy_indexes is not None:
            for player, index in pure_strategy_indexes.items():
                if not isinstance(player, int) and player >= 0:
                    raise ValueError(
                        "Keys in pure_strategy_indexes must be player numbers (ints) >= 0"
                    )
                if not isinstance(index, int) and index >= 0:
                    raise ValueError(
                        "Values in pure_strategy_indexes must be strategy indexes (ints) >= 0"
                    )

        self.id = strategy_id
        self.metadata = metadata
        self._pure_strategy_indexes = (
    ...
    def from_dict(cls, serialized_dict: dict):
        spec = StrategySpec(
            strategy_id=serialized_dict["id"], metadata=serialized_dict["metadata"]
        )
        for player, index in serialized_dict["pure_strategy_indexes"].items():
            spec.assign_pure_strat_index(
                player=int(player), pure_strat_index=int(index)
            )
        return spec
