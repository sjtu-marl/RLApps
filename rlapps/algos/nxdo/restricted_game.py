...
class RestrictedGame(ValidActionsMultiAgentEnv):
    def __init__(self, env_config: dict):
        ...
        self._use_delegate_policy_exploration = env_config.get(
            "use_delegate_policy_exploration", False
        )
    ...
    def set_action_conversion(
        self, agent_id: AgentID, converter: RestrictedToBaseGameActionSpaceConverter
    ):
        self.agents_to_action_converters[agent_id] = converter

    def reset(self) -> MultiAgentDict:
        if (
            self._raise_if_no_restricted_players
            and len(self.agents_to_action_converters) == 0
        ):
        ...
        for agent_id, action in action_dict.items():
            if agent_id in self.agents_to_action_converters:
                convertor: RestrictedToBaseGameActionSpaceConverter = (
                    self.agents_to_action_converters[agent_id]
                )
