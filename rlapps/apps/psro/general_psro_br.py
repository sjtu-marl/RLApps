...
def set_best_response_active_policy_spec_and_player_for_all_workers(
    trainer: Trainer, player: int, active_policy_spec: StrategySpec
):
...
def sync_active_policy_br_and_metanash_with_p2sro_manager(
    trainer: DQNTrainer,
    player: int,
    metanash_player: int,
    ...
    p2sro_manager.submit_new_active_policy_metadata(
        ...
        metadata_dict=create_metadata_with_new_checkpoint_for_current_best_response(
            trainer=trainer,
            player=player,
            save_dir=checkpoint_dir(trainer),
            timesteps_training_br=timesteps_training_br,
            episodes_training_br=episodes_training_br,
...
def train_psro_best_response(
    player: int,
    results_dir: str,
    scenario_name: str,
    psro_manager_port: int,
    ...
    if single_agent_symmetric_game and player != 0:
        if player is None:
            ...
            raise ValueError(
                f"If treating the game as single agent symmetric, only use player 0 "
                f"(one agent plays all sides)."
            )

    p2sro = scenario.p2sro
    p2sro_sync_with_payoff_table_every_n_episodes = (
        scenario.p2sro_sync_with_payoff_table_every_n_episodes
    )
    ...
    class P2SROPreAndPostEpisodeCallbacks(DefaultCallbacks):
        ...
        def on_episode_start(
            ...
            **kwargs,
            ...
            opponent_policy_distribution: PolicySpecDistribution = (
                worker.opponent_policy_distribution
            )
            time_for_resample = (
                not hasattr(metanash_policy, "episodes_since_resample")
                or metanash_policy.episodes_since_resample
                >= resample_pure_strat_every_n_episodes
            )
            if time_for_resample and opponent_policy_distribution is not None:
                new_pure_strat_spec: StrategySpec = (
                    opponent_policy_distribution.sample_policy_spec()
                )
        ...
        def on_episode_end(
            ...
            **kwargs,
            ...
            br_policy_spec: StrategySpec = worker.policy_map[
                "best_response"
            ].policy_spec
    ...
    active_policy_spec: StrategySpec = p2sro_manager.claim_new_active_policy_for_player(
        player=player,
        new_policy_metadata_dict=create_metadata_with_new_checkpoint_for_current_best_response(
            trainer=trainer,
            player=player,
            save_dir=checkpoint_dir(trainer),
    ...
    set_best_response_active_policy_spec_and_player_for_all_workers(
        trainer=trainer, player=player, active_policy_spec=active_policy_spec
    )
    ...
    while True:
        ...
        episodes_since_last_sync_with_manager += train_iter_results[
            "episodes_this_iter"
        ]
        if (
            p2sro
            and episodes_since_last_sync_with_manager
            >= p2sro_sync_with_payoff_table_every_n_episodes
        ):
            if p2sro_sync_with_payoff_table_every_n_episodes > 0:
                episodes_since_last_sync_with_manager = (
                    episodes_since_last_sync_with_manager
                    % p2sro_sync_with_payoff_table_every_n_episodes
                )
            else:
        ...
        if stopping_condition.should_stop_this_iter(
            ...
            if p2sro_manager.can_active_policy_be_set_as_fixed_now(
                player=player, policy_num=active_policy_num
            ):
                break
            else:
                log(
                    f"Forcing training to continue since lower policies are still active."
                )
    ...
    if not p2sro:
        ...
        for player_to_wait_on in range(2):
            wait_count = 0
            while True:
                if p2sro_manager.is_policy_fixed(
...
if __name__ == "__main__":
    ...
    manager_log_dir = RemoteP2SROManagerClient(
        n_players=2,
