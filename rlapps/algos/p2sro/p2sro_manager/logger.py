...
class P2SROManagerLogger(ABC):
    ...
    def on_new_active_policy(
        self, player: int, new_policy_num: int, new_policy_spec: StrategySpec
    ):
        pass

    def on_new_active_policy_metadata(
        self, player: int, policy_num: int, new_policy_spec: StrategySpec
    ):
        pass
...
class SimpleP2SROManagerLogger(P2SROManagerLogger):
    ...
    def __init__(self, p2sro_manger, log_dir: str):
        ...
        self._payoff_table_checkpoint_dir = os.path.join(
            self._log_dir, "payoff_table_checkpoints"
        )
    ...
    def on_active_policy_moved_to_fixed(
        ...
        self._latest_numbered_payoff_table_checkpoint_path = os.path.join(
            self._payoff_table_checkpoint_dir,
            f"payoff_table_checkpoint_{self._payoff_table_checkpoint_count}.json",
        )
        self._latest_numbered_policy_nums_path = os.path.join(
            self._payoff_table_checkpoint_dir,
        ...
        policy_nums_paths = [
            os.path.join(
                self._payoff_table_checkpoint_dir, f"policy_nums_checkpoint_latest.json"
            ),
            self._latest_numbered_policy_nums_path,
        ]

        for pt_checkpoint_path, policy_nums_path in zip(
            pt_checkpoint_paths, policy_nums_paths
        ):
        ...
        checkpoints_manifest_path = os.path.join(
            self._payoff_table_checkpoint_dir, "checkpoints_manifest.json"
        )
        ensure_dir(file_path=checkpoints_manifest_path)
        with open(checkpoints_manifest_path, "a+") as manifest_file:
            if all(
                len(fixed_policy_nums) > 0
                for fixed_policy_nums in fixed_policy_nums_per_player
            ):
                highest_fixed_policies_for_all_players = min(
                    max(fixed_policy_nums)
                    for fixed_policy_nums in fixed_policy_nums_per_player
                )
            else:
                highest_fixed_policies_for_all_players = None
            manifest_json_line = json.dumps(
                {
                    "payoff_table_checkpoint_num": self._payoff_table_checkpoint_count,
