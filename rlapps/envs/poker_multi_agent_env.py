...
def parse_discrete_poker_action_from_continuous_space(
    continuous_action, legal_actions_list, total_num_discrete_actions_including_dummy
):
    assert (
        isinstance(continuous_action, (float, np.floating))
        or isinstance(continuous_action, np.ndarray)
        and isinstance(continuous_action[0], np.floating)
    ), (
    ...
    assert (
        0.0 - 1e-9 <= player_action <= 1.0 + 1e-9
    ), f"action was: {player_action} before normalization: {continuous_action}"
...
class PokerMultiAgentEnv(ValidActionsMultiAgentEnv):
    def __init__(self, env_config=None):
        ...
        if (
            not isinstance(env_config["dummy_action_multiplier"], int)
            and env_config["dummy_action_multiplier"] > 0
        ):
        ...
        self.openspiel_env = Environment(
            game=self.game_version, discount=1.0, **self.open_spiel_env_config
        )

        self.base_num_discrete_actions = self.openspiel_env.action_spec()["num_actions"]
        self.num_discrete_actions = int(
            self.base_num_discrete_actions * self.dummy_action_multiplier
        )
        ...
        if self._append_valid_actions_mask_to_obs:
            self.observation_length = (
                self.orig_observation_length + self.base_num_discrete_actions
            )
        else:
    ...
    def step(self, action_dict):
        ...
        if self._continuous_action_space or (
            ...
            and curr_player_id in self._individual_players_with_continuous_action_space
        ):
        ...
        if player_action not in legal_actions:
            ...
            legal_actions_mask[legal_actions] = 1.0
            raise ValueError(
                f"illegal actions are not allowed.\n"
        ...
        if done:
            ...
            assert (
                self.curr_time_step.rewards[new_curr_player_id] == 0
            ...
            rewards = {
                self.player_map(new_curr_player_id): self.curr_time_step.rewards[
                    new_curr_player_id
                ]
            }
