...
class _PokerAndOshiBetaTorchDist(TorchBeta):
    def __init__(self, inputs, model):
        super(_PokerAndOshiBetaTorchDist, self).__init__(
            inputs, model, low=-1.0, high=1.0
        )


_global_registry.register(
    RLLIB_ACTION_DIST, "PokerAndOshiBetaTorchDist", _PokerAndOshiBetaTorchDist
)
_global_registry.register(
    RLLIB_ACTION_DIST, "TorchGaussianSquashedGaussian", TorchGaussianSquashedGaussian
)
...
def psro_leduc_ppo_params(env: MultiAgentEnv) -> Dict[str, Any]:
    return merge_dicts(
        GRL_DEFAULT_POKER_PPO_PARAMS,
        {
            ...
            "metrics_num_episodes_for_smoothing": 20000,
            "model": merge_dicts(
                MODEL_DEFAULTS,
                {
                    "fcnet_activation": "relu",
